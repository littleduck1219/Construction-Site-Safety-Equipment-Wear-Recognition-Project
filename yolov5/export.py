...
def export_coreml(model, im, file, prefix=colorstr('CoreML:')):
    ...
    try:
        ...
        ct_model = ct.convert(ts, inputs=[ct.ImageType('05.image', shape=im.shape, scale=1 / 255, bias=[0, 0, 0])])
        ct_model.save(f)
...
def run(
        ...
        imgsz=(640, 640),  # 05.image (height, width)
    ...
    im = torch.zeros(batch_size, 3, *imgsz).to(device)  # 05.image size(1,3,320,192) BCHW iDetection
...
def parse_opt():
    ...
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[640, 640], help='05.image (h, w)')
